...
class TemplateClassTest(unittest.TestCase):
    ...
    def test_init(self):
        template = templates.Template(core.Name('x'), [], [])
        self.assertEqual(template.name, core.Name('x'))

    def test_check_param_not_name(self):
        name = core.Name('x')
        notname = core.Value(1)

        with self.assertRaises(error.UnexpectedType):
            templates.Template(notname, [], [])
    ...
    def test_check_param_not_name(self):
        ...
        with self.assertRaises(error.UnexpectedType):
            templates.Template(core.Name('T'), [notparam], [])

    def test_as_triples_empty(self):

        template = templates.Template(core.Name('x'), [], [])

        self.assertEqual(templates.as_triples(template, self.rt), [])
